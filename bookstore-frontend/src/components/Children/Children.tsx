...
const Children = () => {
  return (
    <div className="container mt-4">
      ...
      <div className="row">
        {ChildernBook.map((ChildrenBook, index) => (
          <div className="col-6 col-md-2 col-lg-1-4 mb-3" key={index}>
            <div className="card text-center shadow-sm">
              <img
                src={ChildrenBook.image}
                alt={ChildrenBook.title}
                className="card-img-top"
                style={{ height: "150px", objectFit: "cover" }}
              />
              <div className="card-body">
                <h6 className="card-title">{ChildrenBook.title}</h6>
                <p className="text-muted">
                  <s>{ChildrenBook.discountPrice}</s>
                </p>
                <p className="text-primary fw-bold">{ChildrenBook.price}</p>
